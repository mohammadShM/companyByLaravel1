...
@section('content-me')
    <section class="row m-0">
        ...
        <section class="col-10 offset-1 text-center mt-5">
            <h1 class="font-weight-bolder text-capitalize text-white">All Index Post</h1>
        </section>
        <section class="col-10 offset-1 mt-5 mb-5 pt-5 pl-5 pr-5 pb-4 main-section-for-me">
            <table class="table table-responsive table-hover table-bordered table-dark">
                ...
                @forelse ($post as $value=>$item)
                    <tr>
                        <td>{{$value+1}}</td>
                        <td><img src="{{asset('images/post/'.$item->image)}}" alt="" width="50" height="50"></td>
                        <td>{{$item->alt}}</td>
                        <td>{{$item->subject}}</td>
                        ...
                        <td><a href="{{route('post.edit',$item->id)}}" class="btn btn-warning">
                                update</a></td>
                        <td>
            ...
            <a class="btn btn-success text-white text-capitalize font-weight-bolder mt-3 mb-3"
               href="{{route('post.create')}}">go to create post website</a>
        </section>
    </section>
@endsection
@section('css-me')
