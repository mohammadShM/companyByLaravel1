    ...
    <section class="row m-0">
        ...
        @endif
            <section class="col-10 offset-1 text-center mt-5">
                <h1 class="font-weight-bolder text-capitalize text-white">All Index Seo</h1>
            </section>
        <section class="col-10 offset-1 mt-5 pt-5 pl-5 pr-5 pb-4 main-section-for-me">
            <table class="table table-hover table-bordered table-dark">
                ...
                @foreach ($seo as $value=>$item)
                    <tr>
                        <td>{{$value+1}}</td>
                        <td>{{$item->title}}</td>
                        <td>{{\Illuminate\Support\Str::limit($item->keywords,100)}}</td>
                        <td>{{\Illuminate\Support\Str::limit($item->description,100)}}</td>
                        <td>{{$item->author}}</td>
                        <td><a href="{{route('panel.edit',$item->id)}}"
            ...
            <div class="block">
                <a class="btn btn-success text-white text-capitalize font-weight-bolder mt-3 mb-3"
                   href="{{route('panel.index')}}">go to panel website</a>
            </div>
          <div class="block">
              <a class="btn btn-success text-white text-capitalize font-weight-bolder mt-3 mb-3"
                 href="{{route('panel.create')}}">go to create seo website</a>
          </div>
        </section>
    </section>
@endsection
